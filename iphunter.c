...
volatile sig_atomic_t unchanged = 0;
int daemon_mode = 0;
int delay = 5;
...
void toggle_airplane_mode(int on) {
  ...
  snprintf(cmd1, sizeof(cmd1),
           "su -c \"settings put global airplane_mode_on %s\"", state);

  char cmd2[94];
  snprintf(cmd2, sizeof(cmd2),
           "su -c \"am broadcast -a android.intent.action.AIRPLANE_MODE --ez "
           "state %s > /dev/null 2>&1\"",
           broadcast_state);
...
int get_active_interface_and_ip(char *iface, size_t iface_len, char *ip,
                                size_t ip_len) {
...
void wait_for_network() {
  ...
  while (timeout < NETWORK_READY_TIMEOUT && running) {
    if (get_active_interface_and_ip(iface, sizeof(iface), ip, sizeof(ip)) ==
        0) {
      if (strcmp(ip, "N/A") != 0) {
        return;
      }
...
void print_usage() {
  log_message("Usage:\n");
  log_message("  Normal mode: iphunter [delay] [targets]\n");
  log_message("  Daemon mode: iphunter -d [delay] [targets]\n");
  log_message("\nExamples:\n");
  log_message("  iphunter 5 \"10.21.*;192.168.*\"\n");
  log_message("  iphunter -d 15 \"10.2[1-3].*;192.168.[0-9]+\"\n");
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc > 1 && strcmp(argv[1], "-d") == 0) {
    ...
    if (argc > 2)
      delay = atoi(argv[2]);
    if (argc > 3)
      parse_targets(argv[3], targets, &target_count);
  } else if (argc > 1 &&
    ...
    if (argc > 1)
      delay = atoi(argv[1]);
    if (argc > 2)
      parse_targets(argv[2], targets, &target_count);
  }
  ...
  if (daemon_mode) {
    ...
    log_message("[*] Starting IP Hunter\n");
    log_message("[*] Delay: %ds\n", delay);
    if (target_count > 0) {
      log_message("[*] Targets:");
      for (int i = 0; i < target_count; i++)
        log_message(" %s", targets[i]);
  ...
  get_active_interface_and_ip(current_iface, sizeof(current_iface), current_ip,
                              sizeof(current_ip));
  strncpy(last_ip, current_ip, sizeof(last_ip) - 1);

  log_message("[*] Initial IP: %s (%s)\n", current_ip, current_iface);
